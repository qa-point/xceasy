...
- `scripts/check.sh all` — CI-parity entry point. GitHub Actions запускает contracts, schema/redaction/parallel checks, iOS Swift Package consumer build, unit tests framework, внутренний SwiftUI/UIKit integration fixture и aggregate [F13 coverage policy](13_TEST_COVERAGE_RU.md).
...
- `F06-REQ-002`: одна documented command воспроизводит CI checks локально.
- `F06-REQ-003`: CI выполняет format/lint, unit, integration/UI, schema/golden, redaction и parallel tests.
...
- `F06-REQ-011`: CI применяет versioned aggregate и critical-file coverage policy и публикует machine-readable evidence.
