...
Следуем Swift API Design Guidelines и существующей структуре проекта. Этот документ уточняет обязательные локальные правила. До подключения SwiftFormat/SwiftLint формат проверяется review; после подключения конфигурационные файлы становятся исполняемым источником истины, но не отменяют конституцию.
...
- Схемы в Markdown-документации оформляются как ASCII внутри fenced-блока `text`; Mermaid не используется, чтобы схема одинаково читалась в GitHub, IDE, terminal и AI-контексте.
